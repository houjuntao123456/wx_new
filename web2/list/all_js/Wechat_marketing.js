    ...
    function addfangan() {
        ...
        div.innerHTML = ' <div class="bode">'
            + '   <img id="suoke' + num + '" class="imgss" src="../images/add_picture.png" alt="" width="100%">'
            + '  </div>'
            + ' <br>'
            + '  <div>'
            + ' <textarea id="wx_txt' + num + '" class="textArea_wx" cols="30" rows="2" name="" placeholder="添加图文介绍详情(可不填)"></textarea>'
            + '</div>'
        document.getElementById("fanganss").appendChild(div);
        document.getElementById("tiaoshu").innerHTML = num;
        num++
    }
    ...
    mui(".cccc").on('tap', '.imgss', function () {
        arra_id = this.getAttribute("id");
    ...
    function shangchuan_picture() {
        document.getElementById(arra_id).setAttribute("src", base64_YS);
