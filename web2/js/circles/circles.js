...
//http://www.htmleaf.com/html5/SVG/201512162908.html  插件地址
...
/*
  ...
  var myCircle = Circles.create({
      ...
      radius: 30,//圆形进度条的半径
      value: 43,//圆形进度条的初始值，默认值为0
      maxValue: 100,//圆形进度条的最大值，默认值为100
      width: 5,//圆环的宽度，如果没有指定，值为10
      text: function (value) { return value + '%'; },  //圆形简单中间显示的文本。
      colors: ['#ffffff', '#0395FF'],//一个颜色数组，第一个颜色是整个圆环的颜色，如果没有指定，值为['#EEE', '#F00']
      duration: 400,//动画的持续时间，默认为500.如果传入0或null，就不会有动画效果。
      wrpClass: 'circles-wrp',//包裹整个圆形进度条的元素的class名称。
      textClass: 'circles-text',//圆形进度条中文本的class名称。
      valueStrokeClass: 'circles-valueStroke',//用于value值的SVG path元素的class名称
      maxValueStrokeClass: 'circles-maxValueStroke', //用于maxValue值的SVG path元素的class名称
      styleWrapper: true,//是否在包裹元素上添加行内(inline)样式，默认值为true。
      styleText: true//是否在文本元素上添加行内(inline)样式，默认值为true。
    });
